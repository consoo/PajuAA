...
int	CPciExpWrapper::CM_ImageCopySafe(int nIndexNum, int **nBuffer)
{
	...
	{
		if(m_iInterlace == 1)		length = m_iWidth * m_iHeight * 4 ;
		else
		{
...
void CPciExpWrapper::CM_Interlace_fusion(unsigned char *pRaw8_out, unsigned char *pRaw8_1, unsigned char *pRaw8_2){		// for MOBIS AVN
	int		x, y, k ;
	for(y=0 ; y<((m_iInterlace == 1)?m_iHeight/2:m_iHeight) ; y++)
	{
...
void CPciExpWrapper::CM_YUV411ToRaw8(unsigned char *p12Bit, unsigned char *pRaw8){		// Only MOBIS AVN
	int		x, y, k ;
	for(y=0 ; y<m_iHeight ; y++)
	{
...
void CPciExpWrapper::CM_YUV422ToRaw8_AVN(unsigned char *p12Bit, unsigned char *pRaw8){
	int		x, y, k ;
	...
	for(y=0 ; y<240 ; y++)
	{
		...
		{			
			...
			pRaw8[y*720*2 + x*2] = ( (( p12Bit[y*720*4 + 4*x+0] & 0xf ) << 4 ) | (( p12Bit[y*720*4 + 4*x+1] & 0xf0 ) >> 4 ) ) ;		
			pRaw8[y*720*2 + x*2+1 ] = ( (( p12Bit[y*720*4 + 4*x+2] & 0xf ) << 4 ) | (( p12Bit[y*720*4 + 4*x+3] & 0xf0 ) >> 4 ) ) ;	
...
int CPciExpWrapper::CM_SetGrabDef(void)
{
	...
	m_iline_cnt = ((m_iInterlace == 1)?m_iHeight/2:m_iHeight) ;
	...
	{
		if (m_iPixelPer2byte == 1){
			ret = CM_RegisterWrite(ADDR_GRAB_DEF,  ( (__int64)(m_iWidth*2*2 | (((m_iInterlace == 1)?m_iHeight/2:m_iHeight)<<16) ) ) | ( (__int64)(m_iWidth * ((m_iInterlace == 1)?m_iHeight/2:m_iHeight)*2*2) << 32) ) ;	
		}
		else {
