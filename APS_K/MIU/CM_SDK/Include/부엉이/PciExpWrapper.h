...
#define FRAME_PIXEL_COUNT_ERR	0x00000010						// by SJ 150828
// i2c 관련 return value
...
//	int		frame_cnt ;			// 0 word
	int		reser0 ;			// 0 word
	int		line_cnt ;			// 1 word
	int		frame_cnt ;			// 2 word
	int		pixel_cnt ;			// 3 word
	int		valid_pix_cnt ;		// 4 word
	int		frm_valid_check ;	// 5 word
	int		reser6[2] ;			// 6,7 word
	int		vol_ch1_2 ;			// 8 word
	int		vol_ch3_4 ;			// 9 word
//	int		reser10[2] ;		// 10,11 word
	int		enc_info;			// 10 word
	int		enc_val;			// 11 word
...
public:
	...
	void CM_YUV422ToRaw8			(unsigned char *pRaw10, unsigned char *pRaw8) ;
	void CM_YUV422ToRaw8_AVN		(unsigned char *pRaw10, unsigned char *pRaw8) ;
	void CM_YUV411ToRaw8			(unsigned char *pRaw10, unsigned char *pRaw8);
	void CM_Interlace_fusion		(unsigned char *pRaw8_out, unsigned char *pRaw8_1, unsigned char *pRaw8_2);
...
//	int CM_SetOutputFormat			(int uiFormat) ;						// by SJ 150828
	int CM_SetOutputFormat		(int uiFormat, int pixelperbyte, int interlace) ;
	...
	//UART
	int UART_Tx_Send				(int data);
	int UART_Rx_Rcv					(void);

	int SetVHsyncReverse			(int value);
	...
	int DS90UB914A_Init				(void);
	int	GEELY_Sony_Avm_Reset		(void);
