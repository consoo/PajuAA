...
public:
	...
	DWORD nSizeReceived;
	bool ComposePacket_oc(BYTE nID, BYTE nType, BYTE nBodyLength, BYTE* arrBody);
	bool ComposePacket(BYTE nID, BYTE nType, BYTE nBodyLength, BYTE* arrBody, BYTE* arrData, int index);
	bool ComposePacketFittingSet(BYTE nID, BYTE nType, BYTE nBodyLength, BYTE* arrBody);
	 BYTE CRC_CalculateCRC8(BYTE* Crc_DataPtr, int Crc_Length, BYTE Crc_StartValue8);

	 //
	 int ProcessPacket2();
	 int ReceiveFrameCheck();
	 /*BYTE* body_buffer = new BYTE[1024];
	 BYTE* packet_buffer = new BYTE[1024];*/
	  BYTE		body_buffer[1024];
	   BYTE		packet_buffer[1024];
	 BYTE		recvBuffer[1024];	 // 읽기 버퍼//BYTE		buff[2048];	 // 읽기 버퍼
